...
def add_scene_animations():
    ...
    animate_flow_arrow("Arrow_upsv_upn", [(4.98, 1.1, 1.40), (5.60, 1.1, 1.40), (5.60, -2.55, 1.20), (12.57, -2.55, 1.20), (12.57, 1.0, 1.40)])
...
def build_field():
    ...
    pipe_path("UPSV_to_UPN_treater_inlet", [
        ...
        (5.60, 1.1, 1.10),    # выход по +X перпендикулярно торцу
        (5.60, -2.55, 1.10),  # поворот на юг (торообразный отвод)
    ...
    #     горизонтально к правому outlet-патрубку, вниз. ---
    pipe_path("UPSV_water_tank_overflow", [
        ...
        (7.80, 0.90, 1.40),   # спуск снаружи бака
        (8.10, 0.90, 1.40),   # подход к outlet-патрубку
        (8.10, 0.90, 1.09),   # подключение к outlet-патрубку
    ], 0.04, MATS["pipe_water"])

    # --- Produced Water: от outlet-фланца бака вниз, затем на север
    #     (под площадкой), на запад к КНС. Без коллизий с оборудованием. ---
    pipe_path("Produced_water_UPSV_to_KNS", [
        (8.10, 0.90, 1.09),   # правый outlet бака (фланец ниже)
        (8.80, 0.90, 1.09),   # выход по +X от бака
        (8.80, 0.90, 0.40),   # спуск под площадку
        (8.80, 3.50, 0.40),   # на север за пределами УПСВ
        (5.52, 3.50, 0.40),   # на запад
        (5.52, 3.50, 0.95),   # подъём до уровня водовода
        (5.52, 12.4, 0.95),   # на север к КНС
    ...
    # Газовая линия ДНС→УПСВ подключается к верхней части separator_2:
    pipe_path("Gas_line_DNS_UPSV", [(-6.3, -0.9, 1.72), (-6.3, 2.4, 1.9), (3.3, 2.4, 1.9), (3.3, -0.7, 1.75)], 0.07, MATS["pipe_gas"])
    add_flange("UPSV_sep2_gas_inlet_flange", (3.3, -0.7, 1.75), (0, 0, -1), 0.07, MATS["pipe_gas"])
    ...
    # Газовая линия UPSV→UPN: стартует от верхнего выхода separator_2
    pipe_path("Gas_line_UPSV_UPN", [(3.3, -0.7, 1.75), (3.3, -0.7, 2.10), (3.3, 3.35, 2.10), (14.5, 3.35, 2.10), (14.5, 1.0, 1.90)], 0.07, MATS["pipe_gas"])
